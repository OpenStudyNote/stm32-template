...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagFace_Mask)
    {
        info->event[info->num] = EVENT_Face_Mask;
        info->num++;
        dataPoints->valueFace_Mask = gizStandardDecompressionValue(Face_Mask_BYTEOFFSET,Face_Mask_BITOFFSET,Face_Mask_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagHand)
    {
        info->event[info->num] = EVENT_Hand;
        info->num++;
        dataPoints->valueHand = gizStandardDecompressionValue(Hand_BYTEOFFSET,Hand_BITOFFSET,Hand_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSpeech)
    {
        info->event[info->num] = EVENT_Speech;
        info->num++;
        dataPoints->valueSpeech = gizStandardDecompressionValue(Speech_BYTEOFFSET,Speech_BITOFFSET,Speech_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueFace_Mask != cur->valueFace_Mask)
    {
        GIZWITS_LOG("valueFace_Mask Changed\n");
        ret = 1;
    }
    if(last->valueHand != cur->valueHand)
    {
        GIZWITS_LOG("valueHand Changed\n");
        ret = 1;
    }
    if(last->valueSpeech != cur->valueSpeech)
    {
        GIZWITS_LOG("valueSpeech Changed\n");
        ret = 1;
    }
    ...
    if(last->valueMQ4 != cur->valueMQ4)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMQ4 Changed\n");
            ret = 1;
        }
    }
    if(last->valueMQ7 != cur->valueMQ7)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMQ7 Changed\n");
            ret = 1;
        }
    }
    if(last->valueMQ135 != cur->valueMQ135)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMQ135 Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Face_Mask_BYTEOFFSET,Face_Mask_BITOFFSET,Face_Mask_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFace_Mask);
    gizStandardCompressValue(Hand_BYTEOFFSET,Hand_BITOFFSET,Hand_LEN,(uint8_t *)devStatusPtr,dataPoints->valueHand);
    gizStandardCompressValue(Speech_BYTEOFFSET,Speech_BITOFFSET,Speech_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSpeech);
    ...
    devStatusPtr->valueMQ4 = exchangeBytes(gizY2X(MQ4_RATIO,  MQ4_ADDITION, dataPoints->valueMQ4)); 
    devStatusPtr->valueMQ7 = exchangeBytes(gizY2X(MQ7_RATIO,  MQ7_ADDITION, dataPoints->valueMQ7)); 
    devStatusPtr->valueMQ135 = exchangeBytes(gizY2X(MQ135_RATIO,  MQ135_ADDITION, dataPoints->valueMQ135)); 
